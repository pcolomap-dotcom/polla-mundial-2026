#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Construye el tablero visual multi-jugador SIN JavaScript (pestañas con CSS
puro: radio-inputs + :checked). Funciona en cualquier visor de HTML, incluido
el que usan los celulares al abrir el archivo desde WhatsApp.
Embebe estrategia/dashboard_data.json."""
...
# Selectores CSS que muestran el board y marcan la pill según el radio activo.
show_sel = ",\n".join(f"#tab{i}:checked ~ .boards #b{i}" for i in range(N))
pill_sel = ",\n".join(f'#tab{i}:checked ~ nav .pill[for="tab{i}"]' for i in range(N))
...
/* radios ocultos que controlan las pestañas */
.tabs{{position:absolute;width:1px;height:1px;opacity:0;pointer-events:none;}}
...
.pill{{border:1px solid var(--panel-line);background:var(--chip);color:var(--fg);
  padding:9px 16px;border-radius:999px;cursor:pointer;font-size:.95rem;font-weight:600;
  transition:.15s;user-select:none;}}
.pill:hover{{border-color:var(--accent);}}
.pill .rk{{opacity:.7;font-weight:700;font-size:.8em;margin-right:5px;}}
{pill_sel}{{background:var(--accent);color:#12231a;border-color:var(--accent);}}

.board{{display:none;}}
{show_sel}{{display:block;}}
...
TIERLABEL = {"candidato": "Candidato real", "longshot": "Opción lejana", "fondo": "Fondo de tabla"}


DEAD = " <b style=\"color:var(--red)\">— eliminado</b>"


def render_board(i, p):
    spec = ""
    for s in p["specials"]:
        cls = "ok" if s["alive"] else "no"
        mark = "✓" if s["alive"] else "✕"
        tail = "" if s["alive"] else DEAD
        spec += f'<li><span class="{cls}">{mark}</span><div>{esc(s["txt"])}{tail}</div></li>'
    bets = ""
    for a in p["apuestas"]:
        if a["diff"]:
            cmp = f'<b style="color:var(--red)">⚡ distinto a {esc(p["ref"])}</b> (él: {esc(a["ref"])})'
            badge = '<span class="badge b-contra">⚡ distinto</span>'
        else:
            cmp = f'igual a {esc(p["ref"])}'
            badge = '<span class="badge b-fav">igual</span>'
        bets += f"""
      <div class="bet">
        <div class="m">{esc(a["match"])}<span class="why">{esc(a["reason"])} · {cmp}</span></div>
        <div class="pick">{fl(a["pick"])} <b>{esc(a["pick"])}</b>{badge}<br>
          <span class="sc">{esc(a["score"])}</span></div>
      </div>"""
    d = p["dream"]
    risk_hi = p["last"] >= 15
    dream_note = ({"candidato": f'En él tu prob. de ganar sube a ~{d["condWin"]}%.',
                   "longshot": "Con tus especiales, te pone a pelear el top-5.",
                   "fondo": "El título está lejos; el objetivo es escalar y no quedar último."}[p["tier"]])
    return f"""
    <section class="board" id="b{i}">
      <div class="head">
        <div>
        ...
        <span class="tierbadge tier-{p["tier"]}">{TIERLABEL[p["tier"]]}</span>
      </div>

      <div class="stats">
        <div class="stat win"><div class="lab">Ganar</div><div class="big tnum">{esc(p["winStr"])}</div></div>
        ...
        <div class="stat {"risk" if risk_hi else ""}"><div class="lab">Último</div><div class="big tnum">{p["last"]}%</div></div>
        <div class="stat"><div class="lab">Puesto medio</div><div class="big tnum">#{p["avgRank"]}</div></div>
      </div>
      <div class="note">🎯 Te diferencias de <b>{esc(p["ref"])}</b> · apuestas
        <b>distintas: {p["ndiff"]} de 4</b>. Cada acierto tuyo en tu escenario que {esc(p["ref"])} falle, te acerca.</div>
      ...
      <div class="cols">
        ...
        <div class="panel dream">
          ...
          <div class="note">Ese cuadro te da <b>+{d["podium"]}</b> de podio. {dream_note}</div>
        </div>
      </div>
      ...
      <div class="panel" style="margin-top:14px">
        ...
        <div style="font-size:.82rem;color:var(--fg-dim);margin-bottom:4px">
          ...
          pierden puntos por fallar, apostar <b>⚡ distinto</b> no cuesta nada y te separa.</div>
        {bets}
      </div>

      <div class="sanm {"hi" if risk_hi else "lo"}">{mdb(p["sanMarino"])}</div>

      <div class="note" style="margin-top:14px"><b>Estilo de juego:</b>
        <span class="styletag">{esc(p["style"]["label"])}</span> — {esc(p["style"]["desc"])}</div>
    </section>"""
...
for sm in data["semis"]:
    sc = "".join(f"<div>{esc(s)}</div>" for s in sm["scores"])
    semis_html += f"""
    <div class="panel">
      <h2>{fl(sm['a'])} {esc(sm['a'])} <span style="color:var(--fg-dim)">vs</span> {esc(sm['b'])} {fl(sm['b'])}</h2>
      <div style="font-size:1.05rem;font-weight:700">Avanza: {esc(sm['a'])} <b class="tnum">{sm['advA']}%</b>
        · {esc(sm['b'])} <b class="tnum">{sm['advB']}%</b></div>
      <div class="mini">{sc}</div>
    </div>"""

camino_rows = "".join(
    f"<tr><td>{fl(c['team'])} {esc(c['team'])}</td><td class='r tnum'>{c['champ']}%</td>"
...
radios = "".join(
    f'<input class="tabs" type="radio" name="tab" id="tab{i}"{" checked" if i == 0 else ""}>'
    for i in range(N))
pills = "".join(
    f'<label class="pill" for="tab{i}"><span class="rk">#{p["rank"]}</span>{esc(p["disp"])}</label>'
    for i, p in enumerate(PLAYERS))
boards = "".join(render_board(i, p) for i, p in enumerate(PLAYERS))
...
<div class="wrap">
  <header>
    ...
    <h1>Tableros de estrategia de la polla</h1>
    <p class="sub"><b>👇 Toca tu nombre</b> para ver tu tablero. Ya se jugaron los cuartos;
      faltan 2 semifinales, el 3er puesto y la final, más los premios especiales. Idea central:
      apostar <b>⚡ distinto</b> a tu competencia (no se pierden puntos por fallar), asumiendo que
      sabemos qué apuesta cada uno —en especial Jaime, el líder— antes de mandar lo nuestro.</p>
  </header>

  <section class="blk shared">{semis_html}
  </section>
  <section class="blk panel">
    ...
    <div class="note" style="margin-top:10px">👑 <b>Jaime</b> (líder, {jaime_win} de ganar) apuesta:
      SF1 → <b>{jaime_sf1}</b>, SF2 → <b>{jaime_sf2}</b>, final y 3º al favorito. Puntos por partido:
      semifinal 3 (acierto) / 6 (exacto); 3er puesto y final 4 / 8.</div>
  </section>

  {radios}
  <nav class="players">{pills}</nav>
  <div class="boards">{boards}</div>

  <p class="foot">Generado por <code>estrategia/agentes_polla.py</code> (Monte Carlo, 60.000
    simulaciones + enumeración exacta del podio). Sin conexión ni JavaScript: funciona en cualquier
    celular. Contra quién se diferencia cada jugador y la fuerza de los equipos son editables en el código.</p>
</div>
"""

# Documento HTML autónomo para el repo.
DOC = ('<!doctype html>\n<html lang="es">\n<head>\n<meta charset="utf-8">\n'
       '<meta name="viewport" content="width=device-width, initial-scale=1">\n'
       f'<title>Polla familiar · Tableros por jugador</title>\n</head>\n<body>\n'
       f'{HTML.split(chr(10), 1)[1]}\n</body>\n</html>\n')
